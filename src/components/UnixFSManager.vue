...
import { ref } from 'vue'
import { useUnixFS, type CIDType } from '@/HeliaApi/useUnixFS'
...
const lsOutput = ref()
const handleGetDirectory = async (cid: CIDType, pathName: string) => {
...
const handleAddFile = async () => {
  const response = await addFile(fileName.value, lsOutput.value[0].cid, fileContent.value)
...
const handleGetDirectoryContents = async () => {
  const response = await getDirectory(fileDirCid.value)
...
<template>
  ...
  <div v-if="dirCid">
    ...
    <p id="dirListOutput" :key="index" v-for="(item, index) in lsOutput">{{ item }}</p>
  </div>
  <div v-if="lsOutput">
  ...
  <div v-if="fileDirCid">
    <button id="dirContentsButton" @click="handleGetDirectoryContents">
      Get Directory Contents
    </button>
    <p id="dirContentsOutput" :key="index" v-for="(item, index) in directoryContents">
